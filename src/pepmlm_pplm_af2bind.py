...
def gibbs_sample_peptide(
    ...
    """
    Conditional Gibbs sampling for peptide generation.

    Parameters
    ----------
    af2bind_prior : AF2BindPrior or None
        None の場合は従来通り PepMLM のみでサンプリング。
    lambda_af2bind : float
        AF2BIND prior を logits に加算するスケール。
        final_logits = pepmlm_logits / T + lambda * af2bind_prior
        0 にすれば AF2BIND の影響はゼロ（ベースラインと等価）。
    """
    ...
    for _ in range(num_steps):
        ...
        # AF2BIND prior は「ペプチド全体の現在状態」に依存するので、
        # 1 step（全位置1巡）に1回だけ計算する方針でも良い。
        # ただし位置ごとに状態が変わるので、毎位置で再計算する方が正確。
        # → まずは毎位置で再計算（コスト低）
        ...
        for pos in positions:
            ...
            if af2bind_prior is not None and lambda_af2bind > 0.0:
                ...
                # -inf との加算は -inf のまま（20AA以外の vocab 位置は抑制される）
...
def main():
    ...
    # AF2BIND prior の初期化
    # =========================
    # config に AF2BIND_MATRIX_PATH が設定されていれば prior を有効化
    af2bind_prior = None
    ...
    if af2bind_path and lambda_af2bind > 0.0:
        ...
        af2bind_prior = AF2BindPrior(
            ...
            tokenizer=tokenizer,
            device=cfg.DEVICE,
            dtype=torch.float32,   # prior計算はfloat32で安定化
            pbind_bias_scale=float(getattr(cfg, "PBIND_BIAS_SCALE", 5.0)),
        )
        print(f"[AF2BIND] prior enabled (lambda={lambda_af2bind}, "
