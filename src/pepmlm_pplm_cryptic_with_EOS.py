...
def load_cryptic_scores(path: str, target_seq: str) -> List[float]:
    """
    cryptic pocket スコアファイル（JSON）を読み込み、
    ターゲット配列の各残基に対応するスコアリストを返す。
    ...
    [
      {
        ...
        "residue_name": "M",
        "raw_score": 0.001,
        "normalized_score": 0.002,
        "active_score_type": "normalized",
        "displayed_score": 0.002
      },
      ...
    ]

    Returns:
        List[float]: len == len(target_seq) の順序付きスコアリスト
    """
    with open(path, "r") as f:
        data = json.load(f)

    # residue_id でソート（1始まり想定）
    ...
    # displayed_score を優先して使用、なければ normalized_score、なければ raw_score
    scores = []
...
# =========================
def _build_recovery_input_ids(
    pep_token_ids: List[int],
    target_token_ids: List[int],         # [CLS] を含まない生トークンID列
    masked_target_token_ids: List[int],  # 重要残基をMASKに置換済み
    cls_id: int,
    sep_id: Optional[int],
) -> torch.Tensor:
    """
    recovery forward 用の input_ids を文字列変換なしで構築する。

    構造: [CLS] pep_ids [SEP] masked_target_ids [SEP]
    SEP がない tokenizer の場合は省略される。

    Returns:
        1D LongTensor
    """
    ids = [cls_id] + pep_token_ids
    if sep_id is not None:
        ids += [sep_id]
    ids += masked_target_token_ids
    if sep_id is not None:
        ids += [sep_id]
...
def gibbs_sample_peptide_smc(
    ...
    """
    SMC誘導Gibbsサンプリング。差分recoveryスコアで「ペプチドが追加した情報量」を測定。

    差分スコア:
        delta_i = log p(target | pep_i + masked_target)
                - log p(target | all_mask_pep + masked_target)

    これによりタンパク質文脈だけで復元できる分をキャンセルし、
    ペプチド固有の寄与のみをSMCスコアとして使う。

    軽量化:
        - ベースライン（全MASKペプチド）のrecovery forwardをステップ先頭で1回だけ実施
        - recoveryバッチ入力をテンソル操作で構築（Pythonループ最小化）
        - 正解トークンのlog確率をgather()でベクトル化
    """
    mask_token_id = tokenizer.mask_token_id
    cls_token_id  = tokenizer.cls_token_id
    sep_token_id  = tokenizer.sep_token_id
    ...
    # EOS を末尾トークンとして使う（sep が None の PepMLM-650M 対応）
    end_token_id = sep_token_id if sep_token_id is not None else eos_token_id
    ...
    if not use_smc:
        ...
        return gibbs_sample_peptide(
            ...
            temperature, device, af2bind_prior, lambda_af2bind
        )

    effective_particles = num_particles if smc_lambda > 0.0 else 1

    # ------------------------------------------------------------------
    # ステップ2: 事前計算（ループ外で1回だけ）
    # ------------------------------------------------------------------
    ...
    # recovery input の構造:
    # [CLS] pep_ids [EOS] masked_target_ids [EOS]
    #   1   pep_len   1      len(target)      1
    # ペプチド部分の絶対位置: 1 (CLS のみ)
    # マスク済みターゲットの先頭絶対位置
    target_offset_in_recovery = 1 + pep_len + 1  # CLS + pep + EOS

    # masked_target 内のMASK位置 → recovery forward での絶対位置
    ...
    # recovery input の固定長
    recovery_seq_len = 1 + pep_len + 1 + len(masked_target_token_ids) + 1

    # recovery input の固定部分（ターゲット側）をテンソルとして事前確保
    # 構造: [..., EOS, masked_target_ids, EOS]
    # ペプチド部分だけをループ内で差し替える
    recovery_fixed_suffix = torch.tensor(
        [end_token_id] + masked_target_token_ids + [end_token_id],
        dtype=torch.long, device=device
    )  # (1 + len(target) + 1,)

    # 通常Gibbs forward のペプチド開始位置
    # 構造: [CLS] target_ids [EOS] pep_ids [EOS]
    gibbs_pep_start = 1 + len(target_token_ids) + 1

    # 通常Gibbs forward の固定prefix（ターゲット側）
    gibbs_prefix = torch.tensor(
        [cls_token_id] + target_token_ids + [end_token_id],
        dtype=torch.long, device=device
    )  # (1 + len(target) + 1,)

    # ------------------------------------------------------------------
    # ベースラインrecovery log prob を計算する関数（全MASKペプチド）
    # ステップ先頭で1回だけ呼ぶ
    # ------------------------------------------------------------------
    def compute_baseline_recovery() -> float:
        """全MASKペプチドを使ったrecovery log prob（ターゲット文脈のみ）"""
        all_mask_pep = torch.full((pep_len,), mask_token_id, dtype=torch.long, device=device)
        row = torch.cat([
            torch.tensor([cls_token_id], dtype=torch.long, device=device),
            all_mask_pep,
            recovery_fixed_suffix,
        ])  # (recovery_seq_len,)
        out = model(input_ids=row.unsqueeze(0)).logits[0]  # (seq_len, vocab)
        # gather で正解トークンのlog確率を一括取得
        logits_at_mask = out[recovery_mask_pos_tensor]  # (n_mask, vocab)
        log_probs = F.log_softmax(logits_at_mask.float(), dim=-1)  # (n_mask, vocab)
    ...
    for _ in range(num_steps):
        ...
        for pos in positions:
            ...
            # ----------------------------------------------------------
            # 3-1. 通常Gibbs forward（候補トークンのサンプリング）
            # ----------------------------------------------------------
            ...
            candidate_log_probs = log_probs_base[candidates]  # (N,)

            # ----------------------------------------------------------
            # 3-2. 差分SMCスコア計算
            #
            # delta_i = log p(target | pep_i) - log p(target | all_mask_pep)
            #
            # バッチ化: (N, recovery_seq_len) をテンソル操作で一括構築
            # ----------------------------------------------------------
            if smc_lambda > 0.0 and len(recovery_mask_abs_positions) > 0:
                # ペプチド部分のベーステンソルをN個コピー
                # (N, pep_len)
                batch_pep = peptide_token_ids_tensor.unsqueeze(0).expand(
                    effective_particles, -1
                ).clone()
                # pos列だけ各候補に差し替え
                batch_pep[:, pos] = candidates  # (N, pep_len)

                # [CLS] を (N, 1) で準備
                cls_col = torch.full((effective_particles, 1), cls_token_id, dtype=torch.long, device=device)
                # suffix を (N, suffix_len) に拡張
                suffix_expanded = recovery_fixed_suffix.unsqueeze(0).expand(effective_particles, -1)

                # (N, recovery_seq_len) = [CLS | pep | EOS | masked_target | EOS]
                batch_input = torch.cat([cls_col, batch_pep, suffix_expanded], dim=1)

                batch_logits = model(input_ids=batch_input).logits  # (N, seq_len, vocab)

                # (N, n_mask, vocab) → (N, n_mask)
                logits_at_mask = batch_logits[:, recovery_mask_pos_tensor, :]  # (N, n_mask, vocab)
                log_probs_at_mask = F.log_softmax(logits_at_mask.float(), dim=-1)  # (N, n_mask, vocab)

                # 正解トークンのlog確率を gather で取得
                ...
                recovery_scores = correct_log_probs.mean(dim=1)  # (N,)

                # 差分スコア: ペプチドの寄与のみ
                delta_scores = recovery_scores - baseline_recovery  # (N,)
...
def make_mlm_collate_fn(pad_token_id: int):
    """
    可変長サンプルをパディングしてバッチ化する collate_fn を返す。
    - input_ids       : pad_token_id で埋める
    - attention_mask  : 0 で埋める（パディング位置はモデルが無視）
    - labels          : -100 で埋める（loss計算から除外）
    """
    def collate(batch):
...
def main():
    ...
    if cryptic_scores_path and os.path.exists(cryptic_scores_path):
        ...
        print(
            f"[SMC] 重要残基数: {n_important} / {len(cryptic_scores)} "
            f"(threshold={cryptic_threshold})"
        )
        print(
            f"[SMC] smc_lambda={smc_lambda}, num_particles={smc_num_particles}"
        )
        use_smc = True
    else:
    ...
    if pep_len_min is not None and pep_len_max is not None:
        ...
        print(
            f"\n[PepLen] 可変長モード: {pep_len_min}〜{pep_len_max} の範囲でランダムサンプリング"
        )
    ...
    for it in range(cfg.NUM_ITERATIONS):
        ...
        checkpoint_folder = os.path.join(
            "/mnt/hdd/morishita/pepmlm_pplm/checkpoint", folder_name
        )
